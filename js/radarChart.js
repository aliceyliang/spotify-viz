...
// Some additional edits added by Alice Liang
...
function RadarChart(id, data, options) {
    ...
    blobWrapper
        ...
        .attr("class", function(d) {
            return "radarArea" + " " + d[0][areaName].replace(/[^A-Z0-9]/ig,'') //Remove special characters from the areaName string to make one valid class name
        })
...
blobWrapper.selectAll(".radarCircle")
  ...
  .style("fill", function(d,i){return cfg.color(_.findIndex(_.map(data, function(d) {return {areaName: d[0][areaName]}}), {areaName : d[areaName]}))})
  .style("fill-opacity", 0.8);
    ...
    function cellover(d) {
            ...
            //Bring back the hovered over blob
            d3.select("." + data[d][0][areaName].replace(/[^A-Z0-9]/ig,'')) 
                .transition().duration(200)
    ...
    var dataLength = [ ...Array(data.length).keys() ];

    var legendOrdinal = d3.legendColor()
  //d3 symbol creates a path-string, for example
  //"M0,-8.059274488676564L9.306048591020996,
  //8.059274488676564 -9.306048591020996,8.059274488676564Z"
    // .shape("path", d3.symbol().type("triangle-up").size(150)())
    .shapePadding(10)
    .scale(d3.scaleOrdinal().domain(dataLength).range(dataLength.map(d => cfg.color(d))))
    // .labels(cfg.color.domain().map(function(d){
    //     return data[d][0][areaName];
    // }))
    .labels(data.map(f => f[0].playlist))
